...
class Theme
{
	...
	public function new(content:String)
	{
		try
		{
			...
			var prefix = '';
			if (access.has.folder)
				prefix = access.att.folder.trim();
			for (element in access.elements)
			{
				var name:String = element.name.trim();
				if (!Type.getInstanceFields(Theme).contains(name))
					continue;
				var field = Reflect.field(this, name);
				if (!(field is ThemeData))
					continue;
				var data = cast(field, ThemeData);

				if (element.has.path)
					data.sprite = '$prefix${element.att.path.trim()}';
				if (element.has.textColor)
					data.textColor = element.att.textColor.getColorFromDynamic();
				if (element.has.font)
					data.font = element.att.font;
				if (element.has.color)
					data.color = element.att.color.getColorFromDynamic();
				if (element.has.fontSize)
					data.fontSize = Std.parseFloat(element.att.fontSize).getDefault(0);
				if (element.has.left)
					data.left = Std.parseFloat(element.att.left).getDefault(0);
				if (element.has.right)
					data.right = Std.parseFloat(element.att.right).getDefault(0);
				if (element.has.top)
					data.top = Std.parseFloat(element.att.top).getDefault(0);
				if (element.has.bottom)
					data.bottom = Std.parseFloat(element.att.bottom).getDefault(0);
				if (element.has.size)
				{
					var split = element.att.size.split(",");
					data.size.set(Std.parseFloat(split[0]).getDefault(data.size.x), Std.parseFloat(split[1]).getDefault(data.size.y));
				}
				if (element.has.margin)
				{
					var split = element.att.margin.split(",");
					data.margin.set(Std.parseFloat(split[0]).getDefault(data.margin.x), Std.parseFloat(split[1]).getDefault(data.margin.y));
				}
				if (element.has.offset)
				{
					var split = element.att.offset.split(",");
					data.offset.set(Std.parseFloat(split[0]).getDefault(data.offset.x), Std.parseFloat(split[1]).getDefault(data.offset.y));
				}
			}
		}
		catch (e)
