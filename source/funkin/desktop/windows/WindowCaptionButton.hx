...
class WindowCaptionButton extends FlxSprite
{
	...
	public function new(window:Window, i:Int)
	{
		...
		loadGraphic(Paths.image(DesktopMain.theme.captionButtons.sprite), true, Std.int(DesktopMain.theme.captionButtons.size.x),
			Std.int(DesktopMain.theme.captionButtons.size.y));
					...
					switch (ID)
					{
						...
						case 1:
							if (window.resizeable)
								(window.maximized ? window.restore : window.maximize)();
						case 2:
