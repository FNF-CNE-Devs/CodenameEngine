...
class Checkbox extends TextOption
{
	...
	public override function update(elapsed:Float)
	{
		if (checkbox.animation.curAnim == null
			|| (checkbox.animation.curAnim.finished || (checkbox.animation.curAnim.reversed && checkbox.animation.curAnim.curFrame <= 0)))
		{
		...
		if (offset != null)
			checkbox.offset.set((offset.x * checkbox.scale.x) + baseCheckboxOffset.x, (offset.y * checkbox.scale.y) + baseCheckboxOffset.y);
	}
