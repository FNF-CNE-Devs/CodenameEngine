...
class StoryMenuState extends MusicBeatState
{
	...
	public function changeDifficulty(change:Int, force:Bool = false)
	{
		...
		var event = event("onChangeDifficulty",
			new MenuChangeEvent(curDifficulty, FlxMath.wrap(curDifficulty + change, 0, weeks[curWeek].difficulties.length - 1), change));
		if (event.cancelled)
			return;
				...
				switch (el.name)
				{
					case 'characters':
						for (k => char in el.nodes.char)
						{
							...
							if (characters[char.att.name] != null)
								continue;
							var charObj:MenuCharacter = {
								...
								xml: char,
								offset: FlxPoint.get(Std.parseFloat(char.getAtt('x')).getDefault(0), Std.parseFloat(char.getAtt('y')).getDefault(0))
							};
