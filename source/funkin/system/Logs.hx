...
class Logs
{
	...
	public static function init()
	{
		...
		{
			__showInConsole(prepareColoredTrace([logText('${infos.fileName}:${infos.lineNumber}: ', CYAN), logText(Std.string(v))], TRACE));
		};
	...
	public static function prepareColoredTrace(text:Array<LogText>, level:Level = INFO)
	{
		...
		var superCoolText = [
			logText('[  '),
			logText('${Std.string(time.getHours()).addZeros(2)}:${Std.string(time.getMinutes()).addZeros(2)}:${Std.string(time.getSeconds()).addZeros(2)}',
				DARKMAGENTA),
			logText('  |'),
			switch (level)
			{
