...
class Strum extends FlxSprite
{
	...
	{
		if (note != null && note.noteAngle != null)
			return note.noteAngle;
		if (noteAngle != null)
			return noteAngle;
		return angle;
	}
	...
	public function updateNotePosition(daNote:Note)
	{
		...
		var offset = FlxPoint.get(daNote.isSustainNote ? ((Note.swagWidth - daNote.width) / 2) : 0,
			(Conductor.songPosition - daNote.strumTime) * (0.45 * FlxMath.roundDecimal(getScrollSpeed(daNote), 2)));
		var realOffset = FlxPoint.get(0, 0);

		daNote.__strumCameras = lastDrawCameras;

		if (daNote.isSustainNote)
			offset.y -= Note.swagWidth / 2;
