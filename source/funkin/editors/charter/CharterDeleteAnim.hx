...
class CharterDeleteAnim extends CharterNote {
	...
	public override function draw() @:privateAccess {
		for (deleteData in deleteNotes) {
			y = deleteData.note.y + (deleteData.time>deleteTime*.6 ? (deleteData.time/deleteTime)*FlxG.random.float(-1.5, 1.5) : 0); // lunar when no shake :(( 
			x = deleteData.note.x + (deleteData.time>deleteTime*.6 ? (deleteData.time/deleteTime)*FlxG.random.float(-1.5, 1.5) : 0); // lunar when no shake :((
			...
			sustainSpr.exists = deleteData.note.susLength != 0; sustainSpr.alpha = .4;
			...
			for (member in [this, sustainSpr, typeText]) 
				member.alpha *= deleteData.time/deleteTime;
